...
15. [Graph & Diagram Rendering](#15-graph--diagram-rendering-toolsrender_graphpy)
...
### 15.8 Embedding in DOCX Content Modules

```js
// In your content module:
const graphPath = path.join(__dirname, '..', '..', 'images', 'q13-photoelectric.png');

// Generate the graph first (run tools/render_graph.py as a build step)
// Then embed:
content.push(C.p("", { spacing: { after: 80 } }));
...
Already installed in the project Python environment (Python 3.13, matplotlib 3.10.9, schemdraw 0.22, svgwrite 1.4.3).
